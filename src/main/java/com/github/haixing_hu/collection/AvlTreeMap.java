...
public class AvlTreeMap<KEY, VALUE> implements Map<KEY, VALUE> {
  ...
  protected final class Node implements Entry<KEY, VALUE> {
    KEY     key;
    VALUE   value;
    Node    left;
    Node    right;
  ...
  public void putAll(final Map<? extends KEY, ? extends VALUE> map) {
    for (final Entry<? extends KEY, ? extends VALUE> entry : map.entrySet()) {
  ...
  final class EntryIterator extends PrivateIterator<Entry<KEY, VALUE>> {
  ...
  final class EntrySet extends AbstractSet<Entry<KEY, VALUE>> {
    ...
    @Override
    public boolean add(final Entry<KEY, VALUE> entry) {
      final KEY key = entry.getKey();
      final VALUE value = entry.getValue();
