...
public class Buffer<T> implements Serializable, Cloneable {
  ...
  public void compact() {
    if (length == 0) {
      ...
      @SuppressWarnings("unchecked")
      final T[] temp = (T[]) Array.newInstance(valueClass,
          length);
  ...
  public void reserve(final int n, final boolean keepContent) {
    if ((buffer == null) || (buffer.length < n)) {
      final T[] newBuffer = (T[]) Array.newInstance(
          valueClass, n);
  ...
  public void assign(final Buffer<T> that) {
    if (this != that) {
      ...
      if ((buffer == null) || (buffer.length < that.length)) {
        ...
        buffer = (T[]) Array.newInstance(valueClass,
            that.length);
      }
  ...
  public T[] toArray() {
    if (length == 0) {
      ...
      final T[] result = (T[]) Array.newInstance(valueClass,
          length);
